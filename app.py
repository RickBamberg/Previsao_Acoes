...
def get_or_update_model(ticker):
    ...
    # CASO 2: O modelo EXISTE. Verificar se precisa de atualização.
        ...
        # Se o modelo é de dias anteriores, faça o fine-tuning.
            ...
            try:
                # Carrega o scaler e modelo antigos primeiro
                scaler = joblib.load(scaler_path)
                modelo = load_model(model_path)

                # Baixa APENAS os dados novos (com uma margem de segurança)
                # Pega alguns dias antes da data do modelo para garantir continuidade
                buffer_days = 5
                start_date = model_mtime - datetime.timedelta(days=buffer_days)
                start_date_str = start_date.strftime('%Y-%m-%d')
                
                print(f"Baixando dados de {start_date_str} até {today_str}")
                
                novos_dados = yf.download(ticker, start=start_date_str, end=today_str, auto_adjust=True)

                # Verifica se há dados suficientes
                if novos_dados.empty or len(novos_dados) < tamanho_sequencia + 1:
                    print("Dados insuficientes para fine-tuning. Usando modelo existente.")
                    return modelo, scaler, None
                
                # Verifica se a coluna 'Close' existe
                if 'Close' not in novos_dados.columns:
                    print("Coluna 'Close' não encontrada. Usando modelo existente.")
                    return modelo, scaler, None

                # Prepara os dados para o fine-tuning
                dados_fechamento = novos_dados['Close'].values.reshape(-1, 1)
                
                # Verifica se há dados suficientes após o reshape
                if len(dados_fechamento) < tamanho_sequencia + 1:
                    print("Dados insuficientes após processamento. Usando modelo existente.")
                    return modelo, scaler, None

                # ATENÇÃO: usa o scaler ANTIGO para transformar os novos dados
                dados_scaled = scaler.transform(dados_fechamento)

                X_new, y_new = [], []
                for i in range(tamanho_sequencia, len(dados_scaled)):
                    X_new.append(dados_scaled[i-tamanho_sequencia:i, 0])
                    y_new.append(dados_scaled[i, 0])

                # Verifica se há dados suficientes para treinar
                if len(X_new) == 0 or len(y_new) == 0:
                    print("Nenhuma sequência nova para treinar. Usando modelo existente.")
                    return modelo, scaler, None
                ...
                # Em caso de erro, carrega o modelo existente
                try:
                    modelo = load_model(model_path)
                    scaler = joblib.load(scaler_path)
                    return modelo, scaler, None
                except Exception as e2:
                    return None, None, f"Erro ao carregar modelo existente para {ticker}: {e2}"
        ...
        if not ticker:
            ...
            if error_msg:
                ...
                try:
                    ...
                    # Verifica se há dados suficientes
                    if ultimos_dados.empty or len(ultimos_dados) < 60:
                        prediction_result = f"Erro: Dados insuficientes para fazer previsão para {ticker}."
                    else:
                    ...
                    prediction_result = f"Erro ao fazer previsão para {ticker}: {str(e)}"
